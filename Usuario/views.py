...
from Usuario.models import Hash_Senha_Cofre

from Usuario.models import SenhaCriptografada, Arquivo
...
def index(request):
    ...
    texto_criptografado = criptografar_texto(key,"texto_pra_criptografar")
    #texto_descriptografado = descriptografar_texto(key,texto_criptografado)

    return HttpResponse(f'Texto pra criptografar: texto_pra_criptografar <p></p>'
                        f'Texto criptografado: {texto_criptografado} <p></p>'
                        f'Texto descriptografado: {texto_descriptografado}')

    return render(request, "Usuario/home.html")



def descriptografar_texto(key,texto):

    f = Fernet(key.encode(encoding='utf-8'))
    valor = f.decrypt(bytes(texto))
    return str(valor.decode("utf-8"))
...
def inicio(request):
    ...
    return render(request,'Usuario/inicio.html',dados)
...
def inicio_submit(request):
        ...
        try:
            ...
            if (cofre.hash == hash_texto_chave):
                ...
                try:
                    return render(request, 'Usuario/cofre.html',{'senha':key,'cofre':Arquivo.objects.filter(hash__usuario=request.user),

                                                     'senhas_cofre': SenhaCriptografada.objects.filter(usuario=request.user)})
            ...
            dados = {"key":senha}
            print(key)
            return render(request,'Usuario/exibir_senha.html',dados)
...
def criptografar_texto_submit(request):
    ...
    if request.POST:
        ...
        visualizar_senha = request.POST.get('criar')
